...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/07/29 16:44:38 by jupyo             #+#    #+#             */
/*   Updated: 2025/07/29 17:21:05 by jupyo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_surrounded_by_walls(t_game *cub3d, int direction, int i, int j)
{
	...
	if (direction == 3)
		y = -1;
	while (j >= 0 && i >= 0 && cub3d->map[i] && cub3d->map[i][j]
		&& cub3d->map[i][j] != '1')
	{
		if (cub3d->map[i][j] == ' ')
			return (NO);
	...
	return (j >= 0 && i >= 0 && cub3d->map[i] && cub3d->map[i][j]
		&& cub3d->map[i][j] == '1');
}
...
void	check_map(t_game *cub3d, int i, int j)
{
	...
	while (cub3d->map[++i])
	{
		...
		while (cub3d->map[i][++j])
		{
			if (cub3d->map[i][j] != '0' && !ft_isset(cub3d->map[i][j],
					SPAWN_CHARS))
				continue ;
